...
\begin{itemize}
    ...
    \begin{itemize}
        \item Replaces unstructured LINE chat paragraphs with a strict data validation layer.
    \end{itemize}
...
\textbf{Description:} Automatically matches parent/student requests with valid instructors and time slots by evaluating variables as a formal Constraint Satisfaction Problem (CSP). \\
\textbf{Actor:} Admin \\
\textbf{Details:}
\begin{itemize}
    ...
    \begin{itemize}
        \item Evaluates hard limits (instructor availability, branch capacity) to mathematically guarantee conflict-free schedules.
    \end{itemize}
...
\textbf{Details:}
\begin{itemize}
    \item \textbf{Matrix Configuration:}
    \begin{itemize}
    ...
    \item \textbf{Deterministic Buffer Injection:}
    \begin{itemize}
        \item Automatically shifts an instructor's available start time based on the matrix, instantly pruning physically impossible transit paths from the search space without relying on external APIs.
    \end{itemize}
...
\subsubsection{Interactive XAI Decision Dashboard}
\textbf{Description:} A centralized web interface that replaces static spreadsheets, acting as the administration's single source of truth and Explainable AI (XAI) hub. \\
\textbf{Actor:} Admin \\
\textbf{Details:}
\begin{itemize}
    \item \textbf{Instructor Booking List:}
    \begin{itemize}
        \item Displays a structured, real-time list of all teachers and their complete set of booked classes across all dates.
    \end{itemize}
    
    \item \textbf{Decision Trace Logs:}
    \begin{itemize}
        \item Exposes the heuristic scoring of the backend engine, visually explaining why certain times are blocked (e.g., commute violations or capacity limits) to aid staff in client negotiations.
    \end{itemize}
    
    \item \textbf{Operational Filtering:}
    \begin{itemize}
        \item Enables instant filtering by specific instructors, branches, and dates without the visual clutter of standard spreadsheet software.
    \end{itemize}
\end{itemize}
