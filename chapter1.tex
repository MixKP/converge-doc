...
\begin{enumerate}
    ...
    \item \textbf{Constraint Satisfaction AI Engine:} A backend algorithm that models tutoring logistics as a formal Constraint Satisfaction Problem (CSP). It mathematically evaluates multi-dimensional requirements to ensure highly optimized, conflict-free schedules.
    \item \textbf{Inter-Branch Transit Matrix:} A configuration-driven module that utilizes a customizable, admin-defined travel duration matrix between branches. This automatically injects physical commute buffers, instantly pruning the combinatorial search space without relying on external geographic APIs.
    ...
    \item \textbf{Interactive XAI Decision Dashboard:} A centralized web interface that exposes the heuristic scoring of the backend engine, allowing staff to rapidly understand algorithmic decisions and manually negotiate alternative times with clients based on clear logistical data.
