...
Educational timetabling is a highly complex scheduling problem. As the number of instructors, branches, and time slots increases, the possible scheduling combinations grow exponentially, making it very difficult to process\cite{schaerf1999}. 

Rather than relying on a generalized model, Converge defines a specific set of rules tailored to multi-branch tutoring centers. The AI engine enforces strict physical realities (Hard Constraints)—such as ensuring an instructor is scheduled only once at any given time, and verifying that the total number of concurrent bookings does not exceed the fixed physical capacity of a branch. 

As established in timetabling literature\cite{elsakka2015}, the objective of the Converge engine is not to brute-force a single \enquote{perfect} schedule, as that requires too much computing power at scale. Instead, the system focuses on generating valid, conflict-free schedules that strictly follow all hard rules, while doing its best to satisfy administrative preferences (Soft Constraints) using an automated scoring system.

\subsubsection{Travel Buffer System}
Generic SaaS scheduling applications are fundamentally ignorant of physical geography, relying entirely on simple temporal time-blocking. However, experience within the timetabling research community dictates that every single institution operates under unique spatial and operational constraints, rendering off-the-shelf software insufficient for specialized educational logistics\cite{ceschia2022}.

A critical requirement of the Converge architecture is safely dispatching personnel between physical branches without relying on fragile, third-party geographic location APIs. As established by state-of-the-art timetabling benchmarks like the ITC-2019 formulation\cite{ceschia2022}, accounting for specific travel times between physical locations is a foundational requirement for generating valid, real-world schedules. Rather than introducing the unacceptable latency and system dependency of live spatial-routing APIs, Converge addresses this requirement by implementing a deterministic Inter-Branch Transit Matrix.

By calculating commute buffers based on an admin-defined, pre-configured travel duration matrix between branch pairs, the system automatically injects the required transit time into the scheduling engine. This approach instantly filters out physically impossible transit routes, keeping the system fast and preventing human errors caused by missed travel buffers.
